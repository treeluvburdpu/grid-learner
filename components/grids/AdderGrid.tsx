...
export const AdderGrid: React.FC<AdderGridProps> = ({
    mode,
    onReset,
    adderValues,
    onAdderChange
    ...
    for (let r = maxY; r >= 1; r--) {
        ...
        rowElements.push(
            <SquareCellComponent key={headerKey} content={headerVal} actualValue={headerVal}
                ...
                isHeader isLeftHeader
                mode={mode} borderClasses="" baseSizeClasses="w-full h-full" className="text-transparent" />
        );
        ...
        for (let c = 1; c <= maxX; c++) {
            ...
            if (isOdd(c) || c === 8) {
                rowElements.push(
                    <SquareCellComponent key={cellKey} content="" actualValue=""
                        isHovered={false}
                        borderClasses=""
                ...
                rowElements.push(
                    <SquareCellComponent key={cellKey} content={cellContent} actualValue={cellContent as string}
    ...
    return (
        ...
        >
            ...
            <SquareCellComponent
                content="0" actualValue="0" isHovered={hoveredCellKey === "header-corner-0"}
            ...
            {Array.from({ length: maxX }, (_, i) => i + 1).map(num => {
                ...
                return (
                    <SquareCellComponent key={cellKey} content={content} actualValue={content}
                        isHovered={false}
                        isHeader
                        isBottomHeader
                        mode={mode}
