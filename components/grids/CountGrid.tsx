...
export const CountGrid: React.FC<CountGridProps> = ({
  ...
  // Refs to store measurements during render, updated by handleCellMeasure
  const fruitRectsRef = useRef<{ [id: string]: DOMRect }>({});
  const numberRectsRef = useRef<{ [row: number]: DOMRect }>({});

  // Constants for animation
  const ANIMATION_DURATION = 500; // 0.5 seconds

  // Clear refs at the beginning of each render cycle
  // This ensures that fruitRectsRef and numberRectsRef always contain fresh data for the current render
  fruitRectsRef.current = {};
  numberRectsRef.current = {};
  ...
  const handleCellMeasure = useCallback(
    (row: number, col: number, rect: DOMRect) => {
      ...
      if (col < maxX - 3) {
        ...
        const fruitInCell = fruits.find((f) => f.id === selectedFruitId); // Use selectedFruitId for rect association
        if (fruitInCell) {
          fruitRectsRef.current[fruitInCell.id] = rect;
        }
      } else if (col === maxX) {
        // For number line cells
        numberRectsRef.current[row] = rect;
      }
    },
    [fruits, maxX, selectedFruitId] // Add selectedFruitId to dependencies for accurate fruitRectsRef update
  );
  ...
  for (let r = maxY; r >= 1; r--) {
    ...
    for (let c = 1; c <= maxX; c++) {
      ...
      if (c === maxX) {
        ...
        if (r <= currentCount) {
          cellClassName += ' cell-highlight-green-text font-bold'; // Highlight counted numbers
        } else {
          cellClassName += ' cell-mode-adder-darkgrey-text'; // Uncounted numbers
        }
      } else {
        // Grid area for fruits
        if (fruitInCell) {
          cellContent = fruitInCell.type.icon;
          cellClassName += ` fruit-base `; // Apply common fruit styles

          let currentFruitStateClass = 'fruit-inactive'; // Default to inactive

          // Determine subContent and state class based on fruit state
          if (fruitInCell.isCounted) {
            currentFruitStateClass = 'fruit-counted'; // 80% transparent when counted
            subContent = fruitInCell.countValue;
          } else if (fruitInCell.id === selectedFruitId) {
            currentFruitStateClass = 'fruit-selected'; // 100% opaque + ring
            subContent = currentCount; // Display current count for the selected fruit
          } else if (fruitInCell.countValue !== undefined && fruitInCell.countValue !== null) {
            // This fruit has a number attached (i.e., it was previously selected in the chain)
            currentFruitStateClass = 'fruit-active-in-chain'; // 100% opaque
            subContent = fruitInCell.countValue; // Display its assigned count value
          }

          cellClassName += currentFruitStateClass;

          // Add clickable/hover styles if not counted (always interactive if not counted)
          if (!fruitInCell.isCounted) {
            onClickHandler = () => onFruitClick(fruitInCell.id, currentCount + 1);
      ...
      const border = c === maxX ? 'adder-cell-border' : '';
  ...
  useEffect(() => {
    if (selectedFruitId && currentCount > 0) {
      ...
      const fruitRect = fruitRectsRef.current[selectedFruit.id]; // Read from ref
      const numberRect = numberRectsRef.current[currentCount]; // Read from ref
  ...
  }, [selectedFruitId, fruits, onLineComplete, currentCount, maxX, maxY]);
