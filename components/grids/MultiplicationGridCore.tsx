...
export const MultiplicationGridCore: React.FC<MultiplicationGridCoreProps> = ({
    ...
    const formatHeader = useCallback((val: number) => {
        if (mode === 'decimal') {
    ...
    for (let r = maxY; r >= 1; r--) {
        ...
        rowElements.push(
            <SquareCellComponent key={headerKey} content={headerVal} actualValue={headerVal}
                isHovered={hoveredCellKey === headerKey} onMouseEnter={() => handleMouseEnter(headerKey)}
                onClick={() => onSelectLeft(r)}
                isHeader isLeftHeader isSelected={selectedLeft === r}
        ...
        for (let c = 1; c <= maxX; c++) {
            ...
            rowElements.push(
                <SquareCellComponent key={cellKey} content={cellValue} actualValue={cellValue}
                    ...
                    isFirstSelectedRow={isFirstSelRow} isLastSelRow={isLastSelRow}
                    isFirstSelectedCol={isFirstSelCol} isLastSelCol={isLastSelCol}
    ...
    return (
        ...
        >
            ...
            {Array.from({ length: maxX }, (_, i) => i + 1).map(num => {
                ...
                return <SquareCellComponent key={cellKey} content={headerVal} actualValue={headerVal}
                    isHovered={hoveredCellKey === cellKey} onMouseEnter={() => handleMouseEnter(cellKey)}
                    onClick={() => onSelectTop(num)} isHeader isBottomHeader isSelected={selectedTop === num}
